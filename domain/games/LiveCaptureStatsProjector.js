...
function increment(counters, field, amount = 1) {
  ...
  counters[field] = number(counters[field]) + number(amount);
}

/** Project only the stats that can be proven from PBP events. */
  ...
  for (const event of events || []) {
    ...
    switch (action) {
      case "fg2_made":
        ...
        increment(counters, "points", number(event.points) || 2);
        break;
      case "fg2_attempted":
        increment(counters, "fg2_attempted");
        break;
      case "fg3_made":
        ...
        increment(counters, "points", number(event.points) || 3);
        break;
      case "fg3_attempted":
        increment(counters, "fg3_attempted");
        break;
      case "ft_made":
        ...
        increment(counters, "points", number(event.points) || 1);
        break;
      case "ft_attempted":
        increment(counters, "ft_attempted");
        break;
      case "off_reb": increment(counters, "off_reb"); break;
      case "def_reb": increment(counters, "def_reb"); break;
      case "assists": increment(counters, "assists"); break;
      case "steals": increment(counters, "steals"); break;
      case "blocks_made":
        increment(counters, "blocks");
        increment(counters, "blocks_made");
        break;
      case "blocks_received": increment(counters, "blocks_received"); break;
      case "turnovers": increment(counters, "turnovers"); break;
      case "fouls_committed": increment(counters, "fouls_committed"); break;
      case "fouls_drawn": increment(counters, "fouls_drawn"); break;
      default:
        break;
    }
  }
...
/**
 * Build non-destructive baselines for games whose stored BoxScore predates or
...
export function buildPlayerStatBaselines(existingStats = [], loadedEvents = []) {
  ...
  for (const row of existingStats || []) {
    ...
    for (const field of ADDITIVE_FIELDS) {
      baseline[field] = Math.max(0, number(row[field]) - number(fromPbp[field]));
    }
    baseline.minutes = Math.max(0, number(row.minutes));
    baseline.plus_minus = number(row.plus_minus ?? row.plusMinus);
...
function plusMinusFromCurrentEvents(events = [], playerId) {
  let value = 0;
  for (const event of events || []) {
    if (!Array.isArray(event?.onCourt) || !event.onCourt.map(String).includes(String(playerId))) continue;
...
/**
 * Produce the exact snake_case payload accepted by iq_v28_save_game_capture.
  ...
  return (roster || [])
    ...
    .map(player => {
      ...
      const result = {
        ...
        minutes: Math.max(0, number(baseline.minutes)),
        plus_minus: number(baseline.plus_minus) + plusMinusFromCurrentEvents(events, playerId)
      };

      for (const field of ADDITIVE_FIELDS) {
        result[field] = number(baseline[field]) + number(current[field]);
      }
      return result;
    });
}
