/**
 * @fileoverview Fachada segura de informes V37 + informe final de partido V48.
 * @description Conserva evolución y dossier; informe de partido usa lectura RLS actual.
 */
...
export class ReportsView extends LegacyReportsView {
  ...
  _reportContext(extra = {}) {
    ...
    const teamSeasonId = DataStore.getActiveTeamSeasonId?.(teamId)
      ...
      || null;
    return { teamId, teamSeasonId, seasonId: seasonContext?.season_id || seasonContext?.seasonId || null, ...extra };
  }

  _rawPlayers() {
    const teamId = DataStore.getActiveTeamId?.() || null;
    return DataStore.getSeasonParticipantPlayers?.(teamId) || DataStore.getPlayers?.(teamId) || [];
  }
  ...
  _renderDossierModal(players, games) {
    const safe = this.reportAccessPolicy.sanitizeDossierConfig(this.dossierConfig, this._reportContext(), { players, games });
    this.dossierConfig = { ...this.dossierConfig, ...safe.config };
    return super._renderDossierModal(safe.players, safe.games);
  }

  /** La exportación legacy sigue limitada al alcance autorizado. */
  _executeDossierPDFExport() {
    ...
    const safe = this.reportAccessPolicy.sanitizeDossierConfig(this.dossierConfig, context, { players, games });
    const previousConfig = this.dossierConfig;
    this.dossierConfig = { ...previousConfig, ...safe.config };
    try {
      return ReportExporter.withAuthorization(authorization, () => super._executeDossierPDFExport());
    } finally {
  ...
  _applyDossierExportState(container) {
    ...
    button.title = allowed ? "Exportar únicamente información dentro de tu alcance" : "Tu perfil no puede exportar el dossier de temporada";
    if (!allowed) {
  ...
  _ensureEvolutionModeButton(container, containerId) {
    ...
    button.style.cssText = [
      "padding:8px 14px", "border-radius:8px", "border:1px solid #cbd5e1", "cursor:pointer",
      `background:${this.reportMode === "evolution" ? "#1e3a8a" : "#fff"}`,
      `color:${this.reportMode === "evolution" ? "#fff" : "#0f172a"}`, "font-weight:800"
    ].join(";");
    button.addEventListener("click", () => { this.reportMode = "evolution"; this.render(containerId); });
    group.appendChild(button);
  }

  async _loadEvolutionPayload(player) {
    const context = this._reportContext({ playerId: player?.id || null, playerTeamId: player?.team_id || player?.teamId || DataStore.getActiveTeamId?.() || null });
    ...
    let snapshots = [], error = null;
    try {
      snapshots = await this.longitudinalAnalyticsService.listSnapshots({ teamSeasonId: context.teamSeasonId, playerId: player.id, limit: 20 });
    } catch (caught) {
      console.error("[ReportsView] No se pudo cargar evolución longitudinal:", caught);
      error = caught;
    }
    return { allowed: true, context, player, snapshots, snapshot: snapshots[0] || null, error };
  }
  ...
  async _renderEvolutionMode(container, containerId) {
    ...
    if (!this.selectedEvolutionPlayerId || !allowedIds.has(String(this.selectedEvolutionPlayerId))) this.selectedEvolutionPlayerId = String(players[0].id);
    ...
    select.addEventListener("change", event => { this.selectedEvolutionPlayerId = event.currentTarget.value; this.render(containerId); });
    ...
    if (payload.error) {
      content.textContent = `No se pudo cargar el informe de evolución: ${String(payload.error.message || payload.error)}`;
      return;
    }
    ...
    const reportHtml = PlayerEvolutionReportRenderer.render({ player, snapshotRow: payload.snapshot, snapshotHistory: payload.snapshots, teamName: team.name || "", seasonName: season.name || DataStore.getActiveSeasonDisplayName?.(payload.context.teamId) || "" });
    ...
    printButton.title = decision.allowed ? "Imprime exclusivamente la evolución de este jugador" : "Tu perfil puede consultar este informe, pero no exportarlo";
    ...
    if (decision.allowed) {
      printButton.addEventListener("click", () => {
        ...
        ReportExporter.withAuthorization(decision, () => { ReportExporter.printReport(title, reportHtml); });
      });
    }
  }

  /**
   * V48: reemplaza los promedios/posesiones fabricados por un informe de UN partido.
   * Los modos anteriores de jugador, dossier y evolución se conservan sin cambios.
   */
  async _renderFinalGameMode(container, containerId) {
    const content = container.querySelector("#report-view-content-area");
    if (!content) return;
    const context = this._reportContext();
    const games = this.reportAccessPolicy.filterGames(super._getFilteredGames(), context);
    if (this.selectedGameId === "all") {
      content.replaceChildren();
      const title = document.createElement("h2");
      title.textContent = "Informes completos de partido";
      content.append(title);
      const explanatory = document.createElement("p");
      explanatory.textContent = "Selecciona un encuentro para consultar acta, métricas, evaluación y mapas. Cada informe utiliza datos guardados del propio partido.";
      content.append(explanatory);
      for (const game of games) {
        const button = document.createElement("button");
        button.type = "button";
        button.style.cssText = "display:block;width:100%;text-align:left;background:white;border:1px solid #cbd5e1;border-radius:10px;padding:13px;margin:8px 0;cursor:pointer;min-height:44px";
        button.textContent = `${game.date || "Sin fecha"} · ${game.opponent || "Rival"} · ${game.team_score ?? "–"} – ${game.opponent_score ?? "–"} · Abrir informe completo`;
        button.addEventListener("click", () => { this.selectedGameId = String(game.id); this.render(containerId); });
        content.append(button);
      }
      if (!games.length) explanatory.textContent = "No hay partidos disponibles dentro de tus permisos y temporada seleccionada.";
      return;
    }
    const selected = games.find(game => String(game.id) === String(this.selectedGameId));
    if (!selected) {
      content.textContent = "No puedes consultar ese partido desde la temporada o permisos actuales.";
      return;
    }
    const { html, game, context: reportContext, policy } = await loadAuthorizedFinalGameReport({ supabase: this.auth?.supabase, dataStore: DataStore, auth: this.auth, gameId: selected.id });
    // Ignore an asynchronous result if the user already changed game or report mode.
    if (this.reportMode !== "game" || String(this.selectedGameId) !== String(selected.id)) return;
    content.innerHTML = html;
    const print = document.createElement("button");
    print.type = "button";
    print.id = "btn-print-final-game-report";
    print.textContent = "🖨️ Imprimir / guardar informe PDF completo";
    print.style.cssText = "padding:12px 16px;background:#15803d;color:white;border:0;border-radius:9px;font-weight:800;cursor:pointer;min-height:44px";
    print.hidden = !policy.canExport(ReportType.GAME_STATS, reportContext);
    content.prepend(print);
    print.addEventListener("click", () => {
      const decision = policy.authorizeExport(ReportType.GAME_STATS, reportContext);
      if (decision.allowed) ReportExporter.printReport(`Informe_${game.date || "partido"}`, html, { authorization: decision });
    });
  }
  ...
  async render(containerId = "dashboard-content-area") {
    ...
    if (this.reportMode === "evolution") await this._renderEvolutionMode(container, containerId);
    if (this.reportMode === "game") {
      const content = container.querySelector("#report-view-content-area");
      if (content) content.textContent = "Preparando informe con datos persistidos…";
      try {
        await this._renderFinalGameMode(container, containerId);
      } catch (error) {
        console.warn("[ReportsView] Informe no disponible:", error);
        if (content) content.textContent = "No se ha podido recuperar el informe del partido. Reintenta la lectura; no se ha modificado ningún dato.";
      }
    }
  }
}
