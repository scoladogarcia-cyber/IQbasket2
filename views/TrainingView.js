...
export class TrainingView {
  ...
  _renderTrainingForm() {
    ...
    const canEdit=Boolean(editing) && this._canEditTrainingRecord();
    ...
    const duration=this._sessionDuration(editing || {});
    const participants=(editing?.participants || []);
    const selectedIds=participants.map(item => item.player_id);
    const lockedIds=participants
        ...
        <form id="p360-training-form" class="p360-form" data-editing-session-id="${escapeHtml(editing?.id || "")}">
          <div class="p360-form-grid">
            <label>
              ...
              <input type="date" id="p360-training-date"
            ...
            <label class="p360-span-2">
              ...
              <input type="text" id="p360-training-title" maxlength="140"
                value="${escapeHtml(editing?.title || "")}"
                placeholder="Ej. Técnica individual + ventajas 2c1" required />
            </label>
            ...
            <label>
              ...
              <input type="number" id="p360-training-duration" min="1" max="600"
                value="${duration ?? ""}" inputmode="numeric" readonly aria-readonly="true"
                placeholder="Inicio + fin" />
            </label>

            <label>
              <span>${escapeHtml(this.t("player360.training.intensity", "Intensidad prevista 0-10"))}</span>
              <input type="number" id="p360-training-intensity" min="0" max="10" step="0.5"
                value="${editing?.intensity ?? ""}" inputmode="decimal" />
            </label>

            <label class="p360-span-2">
              <span>${escapeHtml(this.t("player360.training.objective", "Objetivo principal"))}</span>
              <textarea id="p360-training-objective" rows="2" maxlength="500"
                placeholder="Qué queremos provocar o mejorar">${escapeHtml(editing?.objective || "")}</textarea>
            </label>
          </div>
            ...
            <div class="p360-subsection-head">
              <div>
                ...
                <small>${escapeHtml(this.t(
                  ...
                  "Los participantes con asistencia ya confirmada quedan protegidos para no borrar histórico por error."
                ))}</small>
              </div>
            </div>
            <div id="p360-training-player-options">
              ${this._renderParticipantChecklist(date,selectedIds,lockedIds)}
            </div>
          </div>
          ...
          <div class="p360-form-actions">
            ...
            <button type="submit" class="p360-primary-btn">
              ${escapeHtml(editing
                ? this.t("common.save_changes", "Guardar cambios")
                : this.t("player360.training.save_session", "Guardar sesión"))}
            </button>
          </div>
        </form>
      </details>
  ...
  _renderExternalForm() {
    const editing=this._editingExternalSession();
    const canCreate=this._can(Permission.CREATE_EXTERNAL_DEVELOPMENT);
    const canEdit=Boolean(editing) && this._canEditExternalRecord();
    if (!canCreate && !canEdit) return "";

    const date=editing?.activity_date || this._defaultDate();
    const bounds=this._dateInputBounds();
    const providerType=editing?.provider_type || EXTERNAL_PROVIDER_TYPE.EXTERNAL_COACH;
    ...
    return `
      <details class="p360-create-panel" id="p360-create-external-panel" ${editing ? "open" : ""}>
        <summary>
          <span>${editing ? "✏️" : "＋"}</span>
          ${escapeHtml(editing
            ? this.t("player360.external.edit", "Editar tecnificación / desarrollo externo")
            : this.t("player360.external.create", "Registrar desarrollo externo"))}
        </summary>
        ...
        <form id="p360-external-form" class="p360-form" data-editing-external-id="${escapeHtml(editing?.id || "")}">
          <div class="p360-form-grid">
            ...
            <label class="p360-span-2">
              <span>Actividad</span>
              <input type="text" id="p360-external-title" maxlength="140"
                value="${escapeHtml(editing?.title || "")}"
                placeholder="Ej. Sesión individual de tiro" required />
            </label>

            <label>
              <span>Código / tipo</span>
              <input type="text" id="p360-external-code" maxlength="80"
                value="${escapeHtml(editing?.activity_code || "")}" placeholder="Ej. SHOOTING" />
            </label>

            <label>
              <span>Proveedor / tecnificador</span>
              <input type="text" id="p360-external-provider" maxlength="140"
                value="${escapeHtml(editing?.provider_name || "")}" placeholder="Nombre opcional" />
            </label>

            <label>
              <span>Duración (min)</span>
              <input type="number" id="p360-external-duration" min="1" max="600"
                value="${editing?.duration_minutes ?? ""}" inputmode="numeric" />
            </label>

            <label>
              <span>Intensidad 0-10</span>
              <input type="number" id="p360-external-intensity" min="0" max="10" step="0.5"
                value="${editing?.intensity ?? ""}" inputmode="decimal" />
            </label>

            <label>
              <span>RPE 0-10</span>
              <input type="number" id="p360-external-rpe" min="0" max="10" step="0.5"
                value="${editing?.rpe ?? ""}" inputmode="decimal" />
            </label>
            ...
            <label>
              ...
              <select id="p360-external-provider-type">
                ${Object.entries(EXTERNAL_PROVIDER_LABELS).map(([value,label]) => `
                  <option value="${escapeHtml(value)}" ${String(value)===String(providerType) ? "selected" : ""}>
                    ${escapeHtml(label)}
                  </option>
                `).join("")}
              </select>
            ...
            <label class="p360-span-2">
              <span>Objetivo</span>
              <textarea id="p360-external-objective" rows="2" maxlength="500"
                placeholder="Qué se trabajó o qué se buscaba mejorar">${escapeHtml(editing?.objective || "")}</textarea>
            </label>

            <label class="p360-span-2">
              <span>Notas</span>
              <textarea id="p360-external-notes" rows="2" maxlength="500"
                placeholder="Observación deportiva opcional">${escapeHtml(editing?.notes || "")}</textarea>
            </label>
          </div>
          ...
          <div class="p360-form-actions">
            ...
            <button type="submit" class="p360-primary-btn">
              ${editing ? "Guardar cambios" : "Guardar desarrollo externo"}
            </button>
          </div>
        </form>
      </details>
