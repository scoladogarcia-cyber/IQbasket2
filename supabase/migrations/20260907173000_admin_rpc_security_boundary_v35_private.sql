-- =============================================================================
-- IQBasket V35 · Administrative RPC privilege boundary · private implementation
--
-- Phase 1 is behavior-neutral: it creates the non-exposed privileged
-- implementation but does not alter any existing public RPC.
-- =============================================================================

begin;

create schema if not exists iq_v35_private;
revoke all on schema iq_v35_private from public, anon, authenticated;
grant usage on schema iq_v35_private to authenticated;

create or replace function iq_v35_private.admin_set_account_status(
  p_user_id uuid,
...
begin
  ...
  if v_target_role is null then
    raise exception 'ACCOUNT_STATUS_USER_NOT_FOUND';
  end if;
  ...
  insert into public.user_account_controls(
    user_id, account_status, status_reason, changed_at, changed_by
  ) values (
    p_user_id, v_status, nullif(trim(coalesce(p_reason,'')), ''), now(), auth.uid()
  )
  ...
  if v_previous is distinct from v_status then
    insert into public.user_account_status_history(
      user_id, previous_status, new_status, reason, actor_id
    ) values (
      p_user_id, v_previous, v_status, nullif(trim(coalesce(p_reason,'')), ''), auth.uid()
    );
  end if;
...
create or replace function iq_v35_private.assign_user_role_context(
  p_user_id uuid,
  p_role text,
...
declare
  ...
  v_standard text[] := array[
    'ENTRENADOR','ANALISTA','PREPARADOR_FISICO','JUGADOR',
    'FAMILIA_TUTOR','VISOR','INVITADO'
  ];
begin
  if auth.uid() is null or not public.iq_account_is_active() then
    raise exception 'ACCOUNT_ACTIVE_AUTH_REQUIRED';
  ...
  if v_role='SUPERADMIN' then
    raise exception 'ROLE_ASSIGNMENT_SUPERADMIN_DENIED';
  end if;

  v_is_super := public.iq_v3_is_global_superadmin();
  if v_is_super then
    if not (v_role='ADMIN' or v_role=any(v_standard)) then
      raise exception 'ROLE_ASSIGNMENT_INVALID';
    end if;
  elsif v_actor_role='ADMIN' then
    if not (v_role=any(v_standard)) then
      raise exception 'ROLE_ASSIGNMENT_PRIVILEGED_DENIED';
    end if;
...
revoke all on function iq_v35_private.admin_set_account_status(uuid,text,text)
  from public, anon, authenticated;
revoke all on function iq_v35_private.assign_user_role_context(uuid,text,uuid,uuid)
  from public, anon, authenticated;
grant execute on function iq_v35_private.admin_set_account_status(uuid,text,text)
  to authenticated;
grant execute on function iq_v35_private.assign_user_role_context(uuid,text,uuid,uuid)
  to authenticated;
