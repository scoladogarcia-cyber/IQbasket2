-- =============================================================================
-- IQBasket v3 · PHASE 3C FULL MIGRATION REHEARSAL · FORCED ROLLBACK
-- Date: 2026-09-02
--
-- IMPORTANT
-- - This file is derived from the exact Phase-3C migration on this branch.
-- - It runs all Phase-3C DDL/backfill/guards inside one PostgreSQL transaction.
-- - It performs assertions before finishing.
-- - It ALWAYS ends with ROLLBACK and contains NO COMMIT.
-- - Run only when no users are actively writing data, because DDL can take locks.
--
-- Expected success indicator:
--   NOTICE: PHASE3C_REHEARSAL_OK ...
--
-- After running it, execute:
--   20260902_verify_v3_phase3c_postrollback_readonly.sql
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- REHEARSAL-ONLY ASSERTIONS (still inside the same transaction)
-- -----------------------------------------------------------------------------
do $$
declare
  membership_count bigint;
  stint_count bigint;
  memberships_without_stint bigint;
  invalid_stats bigint;
  invalid_events bigint;
  duplicate_open_stints bigint;
begin
  if to_regclass('public.roster_membership_stints') is null then
    raise exception 'REHEARSAL_PHASE3C_STINT_TABLE_MISSING';
  end if;

  if to_regprocedure('public.iq_v3_player_eligible_on_date(uuid,uuid,date)') is null
     or to_regprocedure('public.iq_v3_player_participated_in_team_season(uuid,uuid)') is null
     or to_regprocedure('public.iq_v3_roster_admin_capabilities()') is null
    ...
    raise exception 'REHEARSAL_PHASE3C_RPC_MISSING';
  end if;
  ...
  if not exists (
    ...
    raise exception 'REHEARSAL_PHASE3C_STATS_TRIGGER_MISSING';
  end if;
  ...
  if not exists (
    ...
    raise exception 'REHEARSAL_PHASE3C_EVENT_TRIGGER_MISSING';
  end if;
  ...
  if not exists (
    ...
    raise exception 'REHEARSAL_PHASE3C_GAME_TRIGGER_MISSING';
  end if;
  ...
  if not exists (
    ...
    raise exception 'REHEARSAL_PHASE3C_ROSTER_POLICY_MISSING';
  end if;
  ...
  if not exists (
    ...
    raise exception 'REHEARSAL_PHASE3C_STINT_POLICY_MISSING';
  end if;

  select count(*) into membership_count
  from public.roster_memberships;

  select count(*) into stint_count
  from public.roster_membership_stints;

  select count(*) into memberships_without_stint
  from public.roster_memberships rm
  where not exists (
    select 1
    from public.roster_membership_stints rs
  ...
  if memberships_without_stint <> 0 then
    raise exception 'REHEARSAL_PHASE3C_MEMBERSHIPS_WITHOUT_STINT: %',
      memberships_without_stint;
  end if;

  select count(*) into invalid_stats
  from public.player_game_stats pgs
  join public.games g on g.id = pgs.game_id
  where g.team_season_id is not null
    and not public.iq_v3_player_eligible_on_date(
  ...
  if invalid_stats <> 0 then
    raise exception 'REHEARSAL_PHASE3C_INVALID_STATS: %', invalid_stats;
  end if;
  ...
  if invalid_events <> 0 then
    raise exception 'REHEARSAL_PHASE3C_INVALID_EVENTS: %', invalid_events;
  end if;

  select count(*) into duplicate_open_stints
  from (
    select rs.roster_membership_id
    from public.roster_membership_stints rs
    where rs.valid_until is null
    group by rs.roster_membership_id
    having count(*) > 1
  ) duplicates;

  if duplicate_open_stints <> 0 then
    raise exception 'REHEARSAL_PHASE3C_MULTIPLE_OPEN_STINTS: %',
      duplicate_open_stints;
  end if;

  raise notice
    'PHASE3C_REHEARSAL_OK memberships=% stints=% invalid_stats=% invalid_events=%',
    membership_count,
    stint_count,
    invalid_stats,
    invalid_events;
...
-- CRITICAL: rehearsal never persists Phase 3C.
rollback;
