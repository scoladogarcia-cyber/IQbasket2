...
declare
  ...
  v_source_after text;
  v_captured_before uuid;
  v_captured_after uuid;
  v_entries integer;
  v_error text;
  v_caps jsonb;
begin
  ...
  select
    ...
    greatest(
      rms.valid_from,
      least(current_date,coalesce(rms.valid_until,current_date))
    ),
    rms.valid_from-1
  ...
  if v_superadmin is null or v_target_user is null
    ...
    raise exception 'PLAYER360_PHASE4E2_SMOKE_FIXTURE_MISSING';
  end if;
  ...
  -- SUPERADMIN has no sensitive-data bypass.
  ...
  if coalesce((v_caps->>'can_read')::boolean,false) then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_SUPERADMIN_BYPASS';
  end if;
  if coalesce((v_caps->>'external_import_enabled')::boolean,true) then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_EXTERNAL_IMPORT_ENABLED';
  end if;
  if coalesce((v_caps->>'ai_processing_enabled')::boolean,true) then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_AI_ENABLED';
  end if;
  ...
  v_caps:=public.iq_v4e2_wellness_capabilities(
    v_team_season,v_player,'nutrition','PLAYER_SELF_SERVICE'
  );
  if not coalesce((v_caps->>'can_read')::boolean,false)
    ...
    raise exception 'PLAYER360_PHASE4E2_SMOKE_SELF_CAPABILITIES_FAILED';
  end if;

  if (
    select count(*) from public.iq_v4e2_list_wellness_metric_catalog(
      v_team_season,'nutrition'
    )
  )<>4 then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_NUTRITION_CATALOG_INVALID';
  end if;

  if (
    select count(*) from public.iq_v4e2_list_wellness_metric_catalog(
      v_team_season,'recovery'
    )
  )<>5 then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_RECOVERY_CATALOG_INVALID';
  end if;
  ...
  begin
    perform public.iq_v4e2_save_manual_wellness_entry(
      ...
      'PLAYER_SELF_SERVICE',
      jsonb_build_array(
        jsonb_build_object('metric_code','HYDRATION_ADHERENCE','value',3)
      )
    );
    raise exception 'PLAYER360_PHASE4E2_SMOKE_OUTSIDE_STINT_NOT_BLOCKED';
  exception
    when others then
  ...
  v_entry:=public.iq_v4e2_save_manual_wellness_entry(
    ...
    'PLAYER_SELF_SERVICE',
    jsonb_build_array(
      jsonb_build_object('metric_code','HYDRATION_ADHERENCE','value',2),
      jsonb_build_object('metric_code','PRE_TRAINING_FUELING','value',false)
    )
  );

  select source_type,captured_by
    into v_source_before,v_captured_before
  from public.player360_wellness_entries
  where id=v_entry;

  if v_source_before<>'PLAYER_SELF_REPORT'
     or v_captured_before<>v_target_user then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_PROVENANCE_CREATE_FAILED';
  end if;
  ...
  if v_entries<>1 then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_LIST_FAILED';
  end if;
  ...
  perform public.iq_v4e2_save_manual_wellness_entry(
    ...
    'PLAYER_SELF_SERVICE',
    jsonb_build_array(
      jsonb_build_object('metric_code','HYDRATION_ADHERENCE','value',4),
      jsonb_build_object('metric_code','PRE_TRAINING_FUELING','value',true)
    )
  );

  select source_type,captured_by
    into v_source_after,v_captured_after
  from public.player360_wellness_entries
  where id=v_entry;

  if v_source_after<>v_source_before or v_captured_after<>v_captured_before then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_PROVENANCE_UPDATE_FAILED';
  end if;

  if not public.iq_v4e2_archive_wellness_entry(
    v_entry,'PLAYER_SELF_SERVICE'
  ) then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_ARCHIVE_FAILED';
  end if;
  ...
  if v_entries<>0 then
    raise exception 'PLAYER360_PHASE4E2_SMOKE_ARCHIVED_VISIBLE';
  end if;

  raise notice
    'PLAYER360_PHASE4E2_REHEARSAL_SMOKE_OK relation=% authorization=% entry=%',
...
select
  'PLAYER360_PHASE4E2_REHEARSAL_ROLLBACK' as section,
  ...
  to_regclass('public.player360_processing_authorizations') is not null as phase4e_preserved,
  (
  ...
  ) as phase4e2_rehearsal_rollback_clean;
