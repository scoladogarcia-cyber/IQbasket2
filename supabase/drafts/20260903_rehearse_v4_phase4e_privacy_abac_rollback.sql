...
begin
  ...
  if v_superadmin is null or v_target_user is null
    ...
    raise exception 'PLAYER360_PHASE4E_SMOKE_FIXTURE_MISSING';
  end if;
  ...
  if not public.iq_v4e_can_admin_privacy(v_team_season) then
    raise exception 'PLAYER360_PHASE4E_SMOKE_ADMIN_CAPABILITY_FAILED';
  end if;
  ...
  -- Critical invariant: platform superadmin does not read restricted data by role alone.
  if public.iq_v4e_can_access_sensitive_resource(
    v_player,v_team_season,'nutrition','READ','SPORT_PERFORMANCE'
  ) then
    raise exception 'PLAYER360_PHASE4E_SMOKE_SUPERADMIN_BYPASS';
  end if;

  -- Separation of duties: an administrator cannot grant restricted access to itself.
  begin
    ...
    raise exception 'PLAYER360_PHASE4E_SMOKE_SELF_GRANT_NOT_BLOCKED';
  exception
    when others then
  ...
  -- Export is always explicit even for SELF.
  ...
  if (
    ...
    raise exception 'PLAYER360_PHASE4E_SMOKE_AUDIT_MISSING';
  end if;
  ...
  if public.iq_v4e_subject_relation(v_player) <> 'SELF' then
    raise exception 'PLAYER360_PHASE4E_SMOKE_SELF_RELATION_FAILED';
  end if;

  if not public.iq_v4e_can_access_sensitive_resource(
    v_player,v_team_season,'nutrition','EXPORT','PLAYER_SELF_SERVICE'
  ) then
    raise exception 'PLAYER360_PHASE4E_SMOKE_EXPLICIT_EXPORT_DENIED';
  end if;

  if public.iq_v4e_can_access_sensitive_resource(
    v_player,v_team_season,'nutrition','AI_PROCESS','PLAYER_SELF_SERVICE'
  ) then
    raise exception 'PLAYER360_PHASE4E_SMOKE_AI_WITHOUT_OPTIN_ALLOWED';
  end if;

  if public.iq_v4e_can_access_sensitive_resource(
    v_player,v_team_season,'nutrition','READ','SPORT_PERFORMANCE'
  ) then
    raise exception 'PLAYER360_PHASE4E_SMOKE_SELF_PURPOSE_LEAK';
  end if;
  ...
  if public.iq_v4e_can_access_sensitive_resource(
    ...
    raise exception 'PLAYER360_PHASE4E_SMOKE_REVOKED_AUTHORIZATION_STILL_ACTIVE';
  end if;

  raise notice 'PLAYER360_PHASE4E_REHEARSAL_SMOKE_OK relationship=% authorization=% grant=%',
    v_relationship,v_authorization,v_grant;
end
$iq4esmoke$;
...
select
  'PLAYER360_PHASE4E_REHEARSAL_ROLLBACK' as section,
  ...
  ) as phase4e_rehearsal_rollback_clean;
