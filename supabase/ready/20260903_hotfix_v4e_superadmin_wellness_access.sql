-- =============================================================================
-- IQBasket v4 · Hotfix · SUPERADMIN operational access to Nutrition/Recovery
-- Date: 2026-09-03
--
-- Scope:
-- - authenticated global SUPERADMIN only;
-- - existing player/team-season roster scope remains mandatory;
-- - Nutrition + Recovery only;
-- - READ / CREATE / UPDATE only;
-- - SPORT_PERFORMANCE / OPERATIONS only;
-- - EXPORT, AI_PROCESS and Neuro-Cognitive remain on strict ABAC.
--
-- This is an additive policy hotfix. It intentionally does not rewrite the
-- historical Phase 4E migration.
-- =============================================================================

begin;

do $iq4e_hotfix_preconditions$
begin
  if to_regprocedure(
    ...
    raise exception 'SUPERADMIN_WELLNESS_HOTFIX_PREREQUISITES_MISSING';
  end if;
end;
$iq4e_hotfix_preconditions$;
...
begin
  ...
  -- Resource scope remains mandatory for every caller, including SUPERADMIN.
  if not exists (
  ...
  -- SUPERADMIN_OPERATIONAL_WELLNESS_OVERRIDE_V1
  -- Narrow owner/operator exception requested by product policy. It is not a
  -- generic sensitive-data bypass: no export, AI, delete or Neuro-Cognitive.
  if public.iq_v3_is_global_superadmin()
     and v_module in ('nutrition','recovery')
     and v_action in ('READ','CREATE','UPDATE')
     and v_purpose in ('SPORT_PERFORMANCE','OPERATIONS') then
    return true;
  end if;
