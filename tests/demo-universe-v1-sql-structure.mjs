...
assert.match(p360,/insert into public\.training_blocks/i);
assert.match(p360,/insert into public\.training_participants/i);
assert.match(p360,/training_session_id/i);
...
assert.match(verify,/public\.training_blocks\s+b\s+join\s+public\.training_sessions\s+s\s+on\s+s\.id=b\.training_session_id/i);
assert.match(verify,/public\.training_participants\s+p\s+join\s+public\.training_sessions\s+s\s+on\s+s\.id=p\.training_session_id/i);
assert.doesNotMatch(verify,/training_session_blocks/i);
assert.match(rollback,/set edit_state='OPEN'/i);
assert.match(rollback,/DEMO_V1_ROLLBACK_SUPERADMIN_PROFILE_MISSING/);
assert.match(rollback,/DELETE FROM public\.training_blocks/i);
assert.match(rollback,/DELETE FROM public\.training_participants/i);
