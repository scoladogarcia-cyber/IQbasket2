...
if (result.overflow) throw new Error(`Business metrics overflows mobile: ${JSON.stringify(result)}`);
if (result.kpis !== 4) throw new Error(`Business metrics must show four KPIs: ${JSON.stringify(result)}`);
if (!result.text.includes("25%") || !result.text.includes("FAMILY_PRO")) throw new Error(`Business metrics content missing: ${JSON.stringify(result)}`);
console.log(JSON.stringify({ result: "BUSINESS_METRICS_UI_SMOKE_OK", ...result }));
await browser.close();
