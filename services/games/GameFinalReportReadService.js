 ...
 */
import { supabase as configuredSupabase } from "../../config/database.config.js";
...
export async function loadAuthorizedFinalGameReport({ supabase, dataStore, auth, gameId }) {
  ...
  const { data: game, error: gameError } = await client.from("games").select("*").eq("id", gameId).single();
  ...
  const refreshed = await refreshGameBoxScore({ supabase: client, dataStore, gameId });
  ...
  const [{data: periods,error: periodsError},{data: aggregate,error: aggregateError}] = await Promise.all([
    client.from("game_period_scores").select("*").eq("game_id",gameId).order("period_number",{ascending:true}),
    client.from("team_game_stats").select("*").eq("game_id",gameId).maybeSingle()
  ]);
